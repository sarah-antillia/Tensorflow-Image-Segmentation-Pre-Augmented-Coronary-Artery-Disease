...
class ImageMaskDatasetGenerator:
  ...
  def __init__(self, images_dir, json_file, output_dir, 
                rename = True,
                resize = False,
  ...
  def generate_mask_files(self):
    ...
    with open(self.json_file, 'r') as f:
        ...
        self.parse_json(data,  self.output_masks_dir)
        
  # Pleae see:
  # https://www.nature.com/articles/s41597-023-02871-z
  """
  The “annotations” field contains a unique “id” of the annotation, “image_id” value, relating it to the specific image 
  from the “images” field, and a “category_id” relating it to the specific category from the “categories” field. 
  The “segmentation” sub-field contains coordinates of mask edge points in “XYXY” format. 
  """
  def parse_json(self, data,  output_dir ):
      annotations = data["annotations"]
      if annotations == None or type(annotations) !=list:
        return
 
      print("---len {}".format(len(annotations)))

      for anno in annotations:
        mask    = np.zeros((self.SIZE, self.SIZE, 1))

        vertices = anno["segmentation"]
        points = []
        ...
        print("=== Saved {}".format(output_filepath))
        if self.augmentation:
          self.augment(mask, image_name, output_dir, border=(0, 0, 0), mask=True)
...
if __name__ == "__main__":
  try:
    output_dir = "./ARCADE-Stenosis-master"
    if os.path.exists(output_dir):
      shutil.rmtree(output_dir)
    os.makedirs(output_dir)

    train_images_dir = "./segmentation_dataset/seg_train/images/"
    train_json_file  = "./segmentation_dataset/seg_train/annotations/seg_train.json"
    ...
    generator.generate()
    """
    valid_images_dir = "./segmentation_dataset/seg_val/images/"
    valid_json_file  = "./segmentation_dataset/seg_val/annotations/seg_val.json"
    valid_output_dir = output_dir + "/valid" 

    generator = ImageMaskDatasetGenerator(valid_images_dir,  
                                          valid_json_file, 
                                          valid_output_dir,
                                          rename = True,
                                          resize = True,
